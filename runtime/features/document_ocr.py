...
RECEIPT_TAX_TOKENS = ["税", "tax", "税込", "税额", "税額"]
PAYMENT_TOKENS = {
...
class Feature(HomeHubFeature):
    ...
    def run_local_ocr(self, image_base64: str) -> dict:
        if not image_base64 or np is None or Image is None:
            return {}
        ...
        for item in result:
            ...
            if isinstance(item[1], str):
                text = item[1]
            elif isinstance(item[1], list) and item[1]:
                text = str(item[1][0])
            text = text.strip()
            if text:
                lines.append(text)
        ...
        return {
            "summary": lines[0],
            "contentType": "receipt" if any(token in ocr_text.lower() for token in [token.lower() for token in RECEIPT_TOTAL_TOKENS]) else "document",
            "relevantFacts": lines[:20],
            "merchant": lines[0],
            "detectedDate": self.detect_date({"ocrText": ocr_text}),
            "paymentMethod": self.detect_payment_method({"ocrText": ocr_text}),
            "currency": "JPY" if any(token in ocr_text for token in ["円", "日元", "JPY", "¥"]) else "",
            "taxAmount": self.detect_tax_amount({"ocrText": ocr_text}),
            "ocrText": ocr_text,
            "provider": "rapidocr",
    ...
    def append_debug_log(self, runtime: RuntimeBridge, request: dict, local_ocr: dict, raw_model: dict, analysis: dict, receipt_total: int) -> None:
        try:
            entry = {
    ...
    def parse_amount(self, value) -> int:
        ...
        if len(digits_only) >= 8 and not any(token in text.lower() for token in ["¥", "円", "jpy", "元"]):
            return 0
    ...
    def extract_amount_candidates(self, text: str) -> list[int]:
        ...
        if re.search(r"\d{1,2}:\d{2}", cleaned):
            return []
        candidates: list[int] = []
        for match in re.finditer(r"(?:¥\s*|jpy\s*)?(\d{1,6}(?:,\d{3})*)(?:\s*(?:円|元|jpy))?", cleaned, flags=re.IGNORECASE):
            amount = self.parse_amount(match.group(1))
            if amount > 0:
                candidates.append(amount)
        return candidates

    def detect_tax_amount(self, analysis: dict) -> int:
        lines = self.collect_lines(analysis)
        for line in lines:
            if any(token.lower() in line.lower() for token in RECEIPT_TAX_TOKENS):
    ...
    def pick_receipt_total(self, analysis: dict) -> int:
        for candidate in [analysis.get("totalAmount", ""), analysis.get("subtotal", "")]:
            amount = self.parse_amount(candidate)
            if amount > 0:
                return amount
        weighted: list[tuple[int, int]] = []
        for line in self.collect_lines(analysis):
            amounts = self.extract_amount_candidates(line)
            if not amounts:
                continue
            lowered = line.lower()
            for amount in amounts:
                score = 1
                if any(token.lower() in lowered for token in RECEIPT_TOTAL_TOKENS):
                    score = 6
                elif any(token.lower() in lowered for token in RECEIPT_TAX_TOKENS):
                    score = 2
                elif any(token in lowered for token in PAYMENT_TOKENS):
                    score = 5
                if amount >= 1000000:
                    score = -1
    ...
    def collect_lines(self, analysis: dict) -> list[str]:
        lines: list[str] = []
    ...
    def detect_payment_method(self, analysis: dict) -> str:
        ...
        for line in self.collect_lines(analysis):
            lowered = line.lower()
            for token, label in PAYMENT_TOKENS.items():
                if token in lowered or token in line:
                    return label
        return ""
                ...
                normalized_expenses.append(
                    {
                        ...
                        "purchaseDate": str(item.get("purchaseDate", "")).strip(),
                        "paymentMethod": str(item.get("paymentMethod", "")).strip(),
                        "taxAmount": self.parse_amount(item.get("taxAmount", 0)),
                    }
                )
    ...
    def build_analysis_prompt(self, payload: dict, locale: str, document_language: str = "") -> str:
        ...
        return (
            ...
            "Use detectedExpenses as an array of objects with keys amount, category, content, note, merchant, purchaseDate, paymentMethod, taxAmount. "
            "recommendedAction must be one of: record_expense, extract_text_only, ask_follow_up, no_action. "
            "For receipts, prioritize the final payable total and tax lines. "
            "If image understanding is uncertain, preserve OCR text and ask a follow-up instead of hallucinating. "
    ...
    def build_local_only_analysis(self, local_ocr: dict, locale: str) -> dict:
        analysis = self.normalize_analysis(local_ocr)
        analysis["documentLanguage"] = analysis.get("documentLanguage") or detect_document_locale(analysis.get("ocrText", ""), locale)
        total = self.pick_receipt_total(analysis)
        if total > 0:
            analysis["totalAmount"] = total
            analysis["recommendedAction"] = analysis.get("recommendedAction") or "record_expense"
            if not analysis["detectedExpenses"]:
                analysis["detectedExpenses"] = [
                    {
                        "amount": total,
                        "category": "receipt",
                        "content": analysis.get("merchant", "") or "Receipt total",
                        "note": analysis.get("detectedDate", ""),
                        "merchant": analysis.get("merchant", ""),
                        "purchaseDate": analysis.get("detectedDate", ""),
                        "paymentMethod": analysis.get("paymentMethod", ""),
                        "taxAmount": int(analysis.get("taxAmount", 0) or 0),
                    }
                ]
        if not analysis.get("summary"):
            analysis["summary"] = "OCR extracted text from the uploaded image."
        if not analysis.get("recommendedAction"):
            analysis["recommendedAction"] = "extract_text_only" if analysis.get("ocrText") else "ask_follow_up"
        return analysis

    def build_reply(self, analysis: dict, locale: str) -> str:
        if locale == "zh-CN":
            if analysis.get("totalAmount"):
                merchant = str(analysis.get("merchant", "")).strip()
                merchant_text = f" {merchant}" if merchant else ""
                return f"我已经识别这张票据{merchant_text}，目前看到的总金额大约是 {int(analysis['totalAmount'])}。"
            return analysis.get("summary") or "我已经完成图片文字识别。"
        if analysis.get("totalAmount"):
            merchant = str(analysis.get("merchant", "")).strip()
            merchant_text = f" at {merchant}" if merchant else ""
            return f"I reviewed the receipt{merchant_text}. The total I can see is {int(analysis['totalAmount'])}."
        return analysis.get("summary") or "I reviewed the uploaded image."
    ...
    def run_feature(self, runtime: RuntimeBridge, source: str, payload: dict | None = None) -> dict | None:
        ...
        raw_dict: dict = {}
        if runtime.analyze_image:
            prompt = self.build_analysis_prompt(request, locale, document_language)
            if local_ocr.get("ocrText"):
                prompt += f" Local OCR text candidate:\n{local_ocr.get('ocrText', '')}\nUse it as a strong hint for merchant, date, payment method, total, and tax."
            raw = runtime.analyze_image(prompt, image_base64, mime_type, "qwen2.5vl:7b")
            raw_dict = raw if isinstance(raw, dict) else {}

        merged_raw = self.merge_analysis(raw_dict, local_ocr) if raw_dict else local_ocr
        analysis = self.build_local_only_analysis(merged_raw, locale)
        ...
        return {"ok": True, "action": action, "analysis": analysis, "reply": self.build_reply(analysis, locale)}
