...
MEMORY_FILE = Path(__file__).resolve().parent / "data" / "source_reference_memory.json"
MAX_ITEMS = 800
...
def query_source_reference_memory(query_text: str, limit: int = 3, min_score: float = 0.16) -> list[dict[str, Any]]:
...
def remember_source_reference(item: dict[str, Any]) -> dict[str, Any] | None:
    if not isinstance(item, dict):
        return None
    ...
    normalized = {
        "id": str(item.get("id", "")).strip() or f"source-ref-{_now_iso().replace(':', '').replace('-', '')}",
        "title": title,
        "url": url,
        "source": source,
        "queryText": query_text,
        "searchText": str(item.get("searchText", "")).strip() or " ".join(part for part in [query_text, title, source, url] if part),
        "category": str(item.get("category", "")).strip() or "source-reference",
    ...
    for current in items:
        if not isinstance(current, dict):
            continue
        if str(current.get("url", "")).strip() == url:
